...
TILE_CACHE_TIME = 86400 #in seconds
ZOOM_LEVEL = 11
GTILE = 100000004

class Matrix(object):
    def __init__(self, rows, columns, default='1'):
        self.m = []
        for i in range(rows):
...
class BigQuery(object):
    def __init__(self, id, z, q, x, y):
        ...
        self.gql = "WHERE specid = %i AND quad%i = %i" % (int(id),int(z),int(q))
        qu = SpeciesRanges.gql(self.gql)
        rows = qu.fetch(999)
        cur = qu.cursor()
        
        self.b = Matrix(256,256).m
        self.p = ''
        self.img = ''
        ...
        if len(rows)==0: pass
            ...
            while tot==999:
                ...
                for r in rows:
                    ...
                    while i < dim:
                        j = 0
                        while j < dim:
                            self.b[(j+dim*(r.gmapy - Ny[0]))][i+dim*(r.tilex - Nx[0])] = '0'
                            j+=1
                        i+=1
                    tot += 1
                if tot==999:
            ...
            if mt:
                ...
                f = StringIO.StringIO()
                self.p = png.Writer(256,256, greyscale=True, bitdepth=1, transparent=(0xff))
                self.p.write(f,self.b)
                self.img = f.getvalue()
...
class PreCache(webapp.RequestHandler):
  ...
  def post(self):
    ...
    for i in [0,1,2,3]:
        ...
        p = memcache.get(k)
        if p is not None:
            memcache.set(k, p, TILE_CACHE_TIME/z)
        else:
...
class TileQuery(webapp.RequestHandler):
  ...
  def post(self):
    """
    to convert from OSGeo TMS use the following,
    tile_tms_x = tile_google_x
    tile_tms_y = ((1 << zoom) - tile_google_y - 1)
    """
    ...
    y = ((1 << z) - y - 1) if str(m)=='osgeo' else y #convert to gmaps tile numbering if needed
...
class CheckStore(webapp.RequestHandler):
  def get(self):
    ...
    q = SpeciesRanges.gql("WHERE specid = :sid ORDER BY tilex,gmapy", sid=id).fetch(999)
    out = 'Hello<br>'
    b = Matrix(256,256)
    for s in q:
        out += "tile: %s,%s,%s,%s,%s,%s,%s<br>\n" % (s.rowid,s.specid,s.tilex,s.gmapy,s.zoom,s.quad1,s.quad9)
